...
class ShopcartRepository extends ServiceEntityRepository
{
    ...
    public function getUserShopTotal($userid):float
    {
        ...
        SELECT sum(p.Price * s.quantity) as total FROM App\Entity\Shopcart s,App\Entity\Admin\Product p WHERE s.productid=p.id and s.userid=:userid')->setParameter('userid',$userid);
    ...
    public function getUserShopTax($userid):float
    {
        $em = $this->getEntityManager();
        $query = $em->createQuery('
        SELECT ((sum(p.Price * s.quantity))*0.18) as tax FROM App\Entity\Shopcart s,App\Entity\Admin\Product p WHERE s.productid=p.id and s.userid=:userid')->setParameter('userid',$userid);
        $result = $query->getResult();

        if($result[0]["tax"]!=null)
        {
            return $result[0]["tax"];
        } else {
            return 0;
